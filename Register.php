...
input[type=text],input[type=date],input[type=number],input[type=email],input[type=password],input[type=submit],input[type=tel],input[type=textarea] {
    width: 100%;
    padding: 12px;
...
myInput.onkeyup = function() {
  ...
  if(myInput.value.length >= 8) {
	...
	if(isset($_POST["submit"])){
		...
		if ($firstname == "" || $lastname == ""|| $dob == "" || $gender == "" || $address == ""  || $email == "" || $phone == "" || $disease == "" || $username == ""  ||  $password == "")
			$msg = "Please check your inputs!";
		else {
			$sql = $con->query("SELECT Patient_id FROM patient WHERE email='$email'");
			if ($sql->num_rows > 0) {
		...
		$sql="INSERT INTO patient(Firstname,Lastname,DOB,Gender,Address,Email,Phone_No,Disease,Username,Password,isEmailConfirmed,token) VALUES('".$firstname."','".$lastname."','".$dob."','".$gender."','".$address."','".$email."','".$phone."','".$disease."','".$username."','".$password."', '0', '".$token."')";
...
$mail->Body="Please click on the link below:<br><br>
                    <a href='http://localhost/Hospital_Web/confirm.php?email=$email&token=$token'> Click Here </a> ";

$mail->AddAddress($_POST["email"]);
if(($mail->Send()) && ($con->query($sql))){
	echo "<script type='text/javascript'>alert('You have been registered! Please verify your email! for Login!!!!');</script>";
	}
else{
